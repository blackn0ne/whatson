...
const STEPS = [
    { label: 'Requirements', desc: 'Check your server', icon: ListChecks, title: 'Server requirements', subtitle: 'Make sure your server meets the requirements below.' },
    { label: 'Application', desc: 'Name & address', icon: AppWindow, title: 'Application settings', subtitle: 'Basic information about your installation.' },
    { label: 'Database', desc: 'Connect your database', icon: Database, title: 'Database connection', subtitle: 'Enter your database credentials and test the connection.' },
    { label: 'Admin', desc: 'Create your account', icon: ShieldCheck, title: 'Administrator account', subtitle: 'Create the super-admin you will sign in with.' },
    { label: 'Finish', desc: 'Seed & install', icon: Rocket, title: 'Confirm & install', subtitle: 'Choose what to seed, then start the installation.' },
];
...
export default function Setup({ requirements, defaults }) {
    ...
    return (
        ...
        >
            ...
            {step === 0 && <RequirementsStep requirements={requirements} />}
            {step === 1 && <AppSettingsStep data={data} setData={setData} errors={errors} />}
            {step === 2 && (
                <DatabaseStep data={data} setData={setData} errors={errors} onValidityChange={setDbOk} />
            )}
            {step === 3 && <AdminStep data={data} setData={setData} errors={errors} />}
            {step === 4 && <SeedStep data={data} setData={setData} />}
