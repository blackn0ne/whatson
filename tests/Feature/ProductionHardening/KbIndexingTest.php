...
class KbIndexingTest extends TestCase
{
    ...
    public function test_url_document_pointing_at_a_private_or_metadata_address_is_blocked(): void
    {
        // Catch-all stub: if the SSRF guard ever regresses and the request slips
        // through, this returns content we then assert never got indexed — so the
        // test fails loudly instead of hitting the real metadata endpoint.
        Http::fake([
            'api.openai.com/*' => Http::response(['data' => [['embedding' => [0.1, 0.2, 0.3]]]], 200),
            '*' => Http::response('AWS_SECRET_ACCESS_KEY=should-never-be-indexed', 200),
        ]);
        $kb = $this->seedKb();
        ...
        $doc = AiKbDocument::create([
            ...
            'source_type' => 'url',
            'source_ref' => 'http://169.254.169.254/latest/meta-data/iam/security-credentials/',
            'status' => 'pending',
        ]);
        ...
        $this->assertSame(0, $doc->chunks()->count(), 'A private/metadata URL must never be fetched or indexed.');
    }

    public function test_url_document_with_loopback_address_is_blocked(): void
    {
        Http::fake([
            'api.openai.com/*' => Http::response(['data' => [['embedding' => [0.1, 0.2, 0.3]]]], 200),
            '*' => Http::response('internal-admin-page', 200),
        ]);
        $kb = $this->seedKb();
        ...
        $doc = AiKbDocument::create([
            ...
            'title' => 'Loopback',
            'source_type' => 'url',
            'source_ref' => 'http://127.0.0.1/admin',
            'status' => 'pending',
        ]);
        ...
        $this->assertSame(0, $doc->chunks()->count(), 'A loopback URL must never be fetched or indexed.');
    }

    public function test_url_document_with_public_address_is_fetched_and_indexed(): void
    {
        // A literal public IP keeps the test hermetic (no real DNS lookup).
        Http::fake([
            'api.openai.com/*' => Http::response(['data' => [['embedding' => [0.1, 0.2, 0.3]]]], 200),
            '8.8.8.8/*' => Http::response('<html><body>Our refund policy is 30 days.</body></html>', 200),
        ]);
        $kb = $this->seedKb();

        $doc = AiKbDocument::create([
            'kb_id' => $kb->id,
            'title' => 'Public help page',
            'source_type' => 'url',
            'source_ref' => 'http://8.8.8.8/help',
            'status' => 'pending',
        ]);
        ...
        $this->assertSame('indexed', $doc->status);
        $this->assertTrue(
            $doc->chunks()->where('content', 'like', '%refund policy is 30 days%')->exists(),
            'A public URL should be fetched and indexed normally.'
        );
    }
}
